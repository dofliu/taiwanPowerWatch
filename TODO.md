...
- [ ] 備轉容量率即時來源（`sys_dem_sup.csv`）在 GitHub Actions 雲端 CI 的 IP 上會被台電 WAF
      ...
      因應此限制，前端已加「本站估算可調度餘裕」（僅計燃氣/燃煤/核能/汽電共生/燃油/水力/
      抽蓄/儲能，排除看天吃飯的太陽能與風力，避免嚴重高估），但只做顯示，**不接入**供電警戒
      推播（`runners/alert.js` 仍只採信官方 `grid_status.json`）——已驗證純用總裝置容量減目前
      發電量會嚴重高估（曾算出 50%+ 這種不合理數字），此估算仍未計歲修/故障機組容量、也非
      官方「尖峰負載」定義，僅供參考。
